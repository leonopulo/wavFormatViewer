...
typedef struct info_ts{
    /* IARL     ArchivalLocation */
    unsigned long iarlSize;
    unsigned char *iarl;
    /* IART 	Artist */
    unsigned long iartSize;
    unsigned char *iart;
    /* ICMS 	Commissioned */
    unsigned long icmsSize;
    unsigned char *icms;
    /* ICMT' 	Comment */
    unsigned long icmtSize;
    unsigned char *icmt;
    /* ICOP' 	Copyright */
    unsigned long icopSize;
    unsigned char *icop;
    /* ICRD' 	DateCreated */
    unsigned long icrdSize;
    unsigned char *icrd;
    /* ICRP' 	Cropped */
    unsigned long icrpSize;
    unsigned char *icrp;
    /* IDIM' 	Dimensions */
    unsigned long idimSize;
    unsigned char *idim;
    /* IDPI' 	DotsPerInch */
    unsigned long idpiSize;
    unsigned char *idpi;
    /* IENG' 	Engineer */
    unsigned long iengSize;
    unsigned char *ieng;
    /* IGNR' 	Genre */
    unsigned long ignrSize;
    unsigned char *ignr;
    /* IKEY' 	Keywords */
    unsigned long ikeySize;
    unsigned char *ikey;
    /* ILGT' 	Lightness */
    unsigned long ilgtSize;
    unsigned char *ilgt;
    /* IMED' 	Medium */
    unsigned long imedSize;
    unsigned char *imed;
    /* INAM' 	Title */
    unsigned long inamSize;
    unsigned char *inam;
    /* IPLT' 	NumColors */
    unsigned long ipltSize;
    unsigned char *iplt;
    /* IPRD' 	Product */
    unsigned long iprdSize;
    unsigned char *iprd;
    /* ISBJ' 	Subject */
    unsigned long isbjSize;
    unsigned char *isbj;
    /* ISFT' 	Software */
    unsigned long isftSize;
    unsigned char *isft;
    /* ISHP' 	Sharpness */
    unsigned long ishpSize;
    unsigned char *ishp;
    /* ISRC' 	Source */
    unsigned long isrcSize;
    unsigned char *isrc;
    /* ISRF' 	SourceForm */
    unsigned long isrfSize;
    unsigned char *isrf;
    /* ITCH' 	Technician */
    unsigned long itchSize;
    unsigned char *itch;
}info_t;

typedef struct exif_ts{

}exif_t;

typedef struct list_ts{
    unsigned long size;
    struct info_ts *info; // Mast be NULL, if not presend into the chunk.
    struct exif_ts *exif; // Mast be NULL, if not presend into the chunk.
}list_t;
...
private:
    ...
    list_t  list;
    Ui::MainWindow *ui;
    QList<QTableWidgetItem *> *riffChunkNames;
    QList<QTableWidgetItem *> *riffChunk;
    QList<QTableWidgetItem *> *infoChunkNames;
    QList<QTableWidgetItem *> *infoChunk;
    void OpenWavFile();
    QString readChunk(QFile *file);
    void viewChunkInfo(QTreeWidgetItem *item, int column);
};
